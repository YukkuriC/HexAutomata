    ...
    fun load() {
        ...
        ServerEntityEvents.ENTITY_LOAD.register { entity, _ ->
            if (entity is Projectile) {
                ...
                ) CommonEventsHandler.trigger(BuiltinEventMarker.SHOOT, owner, HAEventsFabric.Shoot(entity))
            }
        }
        ...
        ServerLivingEntityEvents.ALLOW_DAMAGE.register { entity, source, damage ->
            ...
            if (source.`is`(DamageTypes.PLAYER_ATTACK)) source.entity.let { it as? ServerPlayer }?.let {
                CommonEventsHandler.trigger(
                    BuiltinEventMarker.MELEE_HIT, it, HAEventsFabric.Hurt(entity, damage.toDouble())
                )
            }
        ...
        ServerLivingEntityEvents.ALLOW_DEATH.register { entity, source, _ ->
            source.entity.let { it as? ServerPlayer }?.let {
                CommonEventsHandler.trigger(BuiltinEventMarker.KILL, it, HAEventsFabric.Kill(entity))
            }
            return@register true
        }
